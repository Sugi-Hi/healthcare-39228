...
●フロント実装：10/2(月)〜10/8(日)新規登録・ログイン、登録更新・ログアウト[ニックネーム(英字のみ)・Email・パスワード]のみデータ入力<個人情報の関係で割愛>

●機能実装 ①：10/6(金)〜10/12(木)バック背景[自然・空など癒されそうな画像|HTML]/(大きさ・上下幅の調整|CSS)、
投稿と同様な回答(ストレス要因<影響度>・生活リフレッシュ習慣<優先度>)
⇒健康状態の表示、提案

●機能実装 ②：10/9(月)〜10/15(日)回答一覧表、詳細ページ
⇒自己分析<マイページ有>・編集(ボタン)・削除(ボタン)

●機能実装 ③：10/10(火)〜10/19(木)詳細ページ
⇒奨励(おススメ)ページ、提案付き
